...
def register_interaction_handler(dp: Dispatcher):
    ...
    async def interaction_handler(call: types.CallbackQuery):
        ...
        await bot.send_message(chat_id= int(chat_id),text=f"{str(get_role_by_user_id(chat_id=int(chat_id), user_id=int(user_id)))} {get_role_by_user_id(chat_id=int(chat_id), user_id=int(user_id)).interaction_message()}")
