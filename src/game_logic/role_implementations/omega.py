...
class Omega(Role):
    ...
    async def send_interactive_messages(self, chat_id: int, bot: Bot):
        ...
        for usr in game.users:
            if usr.role.get_type() == 7:
                user_id = usr.user_id

        inline_markup = types.InlineKeyboardMarkup(row_width=1)

        for user_state in game.users:
            inline_markup.add(types.InlineKeyboardButton(text=user_state.first_name, callback_data=f"{InteractionTypes.switch}_{user_state.user_id}"))
